...
{
    return ota_port_flash_read_data(0, OTA_HASH_START, hash, len);
}
...
#ifndef SUPPORT_CFBB_UPG
static errcode_t upgrade_prepare_flash(uint32_t file_length)
{
    ota_port_flash_block_erase(0, OTA_FOTA_DATA_START, file_length);
    return ERRCODE_SUCC;
}
#endif

static void upgrade_reboot(void)
{
...
#else
    ret = upgrade_prepare_flash(file_length);
#endif
    if (ret != ERRCODE_SUCC) {
        return ERRCODE_FAIL;
    }
...
errcode_t ota_upgrade_request(uint8_t cmd_id, uint8_t *cmd_param, uint16_t cmd_param_size, uint8_t dst)
{
    ...
    ota_port_flash_block_erase(0, OTA_TRANSMIT_INFO_START, OTA_TRANSMIT_INFO_LEN);
    upgrade_reboot();
    return upgrade_send_ack_pkt(cmd_id, (uint8_t *)&ret, sizeof(errcode_t), dst);
}
...
static errcode_t upgrade_ack_resend(uint8_t cmd_id, uint8_t *cmd_param, uint16_t cmd_param_size, uint8_t dst)
{
    ...
    if (hash_size <= 0 || hash_size > OTA_HASH_INFO_SIZE) {
        return ERRCODE_FAIL;
    }
    ...
    if (memcmp((uint8_t *)hash, (uint8_t *)ack_hash, hash_size) == 0) {
        ...
        ota_port_flash_block_erase(0, OTA_TRANSMIT_INFO_START, OTA_TRANSMIT_INFO_LEN);
        ota_port_flash_write_data(0, OTA_HASH_START, ack_hash, OTA_HASH_INFO_SIZE);
    }
    uint8_t return_message[OTA_MSG_LEN] = {0};
    info_ack_t *ret = (info_ack_t *)return_message;
