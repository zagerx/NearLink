...
uint64_t g_pre_print_time = 0;
#define BLE_CBK_PRINT_INTERVAL 500 // 500ms日志打印间隔
...
static uint16_t g_device_mouse_appearance_value = GAP_BLE_APPEARANCE_TYPE_MOUSE;
static uint8_t g_device_mouse_name_value[32] = {'b', 'l', 'e', '_', 'm', 'o', 'u', 's', 'e', '\0'};
static uint8_t g_device_mouse_name_len = 10;
...
static bool bts_compare_mouse_uuid(bt_uuid_t *uuid1, bt_uuid_t *uuid2)
{
...
static void ble_hid_receive_write_req_cbk(uint8_t server_id, uint16_t conn_id, gatts_req_write_cb_t *write_cb_para,
    errcode_t status)
{
    if (uapi_tcxo_get_ms() - g_pre_print_time > BLE_CBK_PRINT_INTERVAL) { // 减少串口打印次数
        g_pre_print_time = uapi_tcxo_get_ms();
        osal_printk("%sReceiveWriteReqCallback--server_id:%d conn_id:%d\n",
...
/* 设置注册服务时的name */
void ble_hid_set_mouse_name(const uint8_t *name, const uint8_t len)
{
...
/* 设置注册服务时的appearance */
void ble_hid_set_mouse_appearance(uint16_t appearance)
{
    g_device_mouse_appearance_value = appearance;
}

/* 初始化HID device */
void ble_hiddev_mouse_server_init(void)
{
...
errcode_t ble_hiddev_mouse_server_send_input_report(const ble_hid_high_mouse_event_st *mouse_data)
{
...
errcode_t ble_hiddev_mouse_server_send_input_report_by_uuid(const ble_hid_high_mouse_event_st *mouse_data)
{
