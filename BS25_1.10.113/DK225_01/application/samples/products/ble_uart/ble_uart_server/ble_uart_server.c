...
static void ble_uart_add_service(void)
{
    ...
    gatts_add_service(BLE_UART_SERVER_ID, &uart_service_uuid, true);//协议栈调用
}
...
/* 添加uart接收服务的所有特征和描述符 */
 void ble_uart_add_rx_characters_and_descriptors(uint8_t server_id, uint16_t srvc_handle)
{
    uint8_t characters_value[] = { 0x66, 0x77 };
