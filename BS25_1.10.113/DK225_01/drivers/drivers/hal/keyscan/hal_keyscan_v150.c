                           ...
                           hal_keyscan_callback_t callback)
{
    ...
    if (keyscan_porting_get_type_sel() == FULL_KEYS_TYPE) {
        ...
        keyscan_porting_pin_set((CONFIG_KEYSCAN_ENABLE_ALL_ROW - 1), (CONFIG_KEYSCAN_ENABLE_ALL_CLO - 1));
    } else if (keyscan_porting_get_type_sel() == SIX_KEYS_TYPE) {
        hal_keyscan_pin_enable(KEYSCAN_ENABLE_THREE_ROW, KEYSCAN_ENABLE_TWO_CLO);
        keyscan_porting_pin_set(KEYSCAN_ENABLE_THREE_ROW, KEYSCAN_ENABLE_TWO_CLO);
    }
...
errcode_t hal_keyscan_v150_resume(uintptr_t param)
{
    ...
    keyscan_porting_pin_set((uint8_t)g_keyscan_suspend_regs[KEYSCAN_ROW_ENABLE],
                            (uint8_t)g_keyscan_suspend_regs[KEYSCAN_COL_ENABLE]);
    hal_keyscan_config_pulse_time(g_keyscan_config.pulse_time);
    hal_keyscan_config_mode(g_keyscan_config.scan_mode);
    hal_keyscan_config_direction(g_keyscan_config.direction);
    ...
    hal_keyscan_regs_set_clk_ena(1);
    hal_keyscan_regs_set_start();
    keyscan_porting_start_suspend_sleeptimer();
    return ERRCODE_SUCC;
}
#endif
