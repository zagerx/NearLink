                                       ...
                                       const hal_spi_extra_attr_t *extra_attr)
{
    ...
    if (attr->ndf > 0) {
        ...
        hal_spi_v100_ctrlr1_set_ndf(bus, 0);
    }
}
...
#pragma weak hal_spi_init = hal_spi_v100_init
errcode_t hal_spi_v100_init(spi_bus_t bus, const hal_spi_attr_t *attr,
...
#pragma weak hal_spi_deinit = hal_spi_v100_deinit
errcode_t hal_spi_v100_deinit(spi_bus_t bus)
{
...
#pragma weak hal_spi_write = hal_spi_v100_write
errcode_t hal_spi_v100_write(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout)
{
...
#pragma weak hal_spi_read = hal_spi_v100_read
errcode_t hal_spi_v100_read(spi_bus_t bus, hal_spi_xfer_data_t *data, uint32_t timeout)
{
...
static errcode_t hal_spi_ctrl_check_fifo_busy(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
    ...
    while (hal_spi_v100_sr_get_tfe(bus) != 1 || hal_spi_sr_get_busy(bus) != 0) {
        if (hal_spi_check_timeout_by_count(&trans_time, timeout)) {
            return ERRCODE_SPI_TIMEOUT;
        }
    }
...
#pragma weak hal_spi_ctrl = hal_spi_v100_ctrl
errcode_t hal_spi_v100_ctrl(spi_bus_t bus, hal_spi_ctrl_id_t id, uintptr_t param)
{
