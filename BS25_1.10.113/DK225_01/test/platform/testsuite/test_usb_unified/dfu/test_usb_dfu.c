...
#ifndef CONFIG_SAMPLE_SUPPORT_SLE_MOUSE_DONGLE
static uintptr_t g_dfu_data = (uintptr_t)FRAME_DOWNLOAD_OFFSET;
#endif
uint32_t g_test_dfu_write_len = 0;
...
#ifdef USE_EMBED_FLASH
    uapi_eflash_erase(FRAME_DOWNLOAD_OFFSET, g_test_dfu_write_len);
#endif
...
void usb_dfu_download_callback(const uint8_t *buf, uint32_t len)
{
    ...
    osal_printk("\ng_test_dfu_write_len:%u  \n", g_test_dfu_write_len);
    uapi_watchdog_kick();
    g_dfu_data += len;
}
#endif