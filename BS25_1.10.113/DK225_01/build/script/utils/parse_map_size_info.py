...
def get_group_owner(data_dict, save_name):
    ...
    with open(OUTPUT_DIR / f"{save_name}_library_summary.csv", 'w', encoding='utf_8_sig') as f:
        ...
        f.write(f"{','.join(header)}\n")
       # 各组device限制 PLAT WIFI BTC BTH UNKNOWN  STACK PKTRAM 
        lim_63 = {'PLAT' : 35.50, 'WIFI':92.8, 'BTC': 20, 'BTH': 10, 'UNKNOWN': 5, 'STACK':5,'PKTRAM':48}
        lim_53 = {'PLAT' : 16.68, 'WIFI':18.64, 'BTC': 25, 'BTH': 10, 'UNKNOWN': 25, 'STACK':4,'HEAP':25}
        lim_i = 0
        for owner, size in group_dict.items():
            size_t = size/BIT_SIZE
            if save_name == 'ws63-liteos-app.map_ram':
                f.write(f"{owner},{size_t},{lim_63[owner]}\n")
                if size_t > lim_63[owner]:
                    print("Error " + owner + " Exceeded device memory size limit!! Please cut to make it smaller! Current size is %.3f limit is %.3f" %(size_t, lim_63[owner]))
                    sys.exit(1)
                lim_i += 1
            elif save_name == 'control_ws53.map_ram':
                f.write(f"{owner},{size_t},{lim_53[owner]}\n")
                if size_t > lim_53[owner]:
                    print("Error " + owner + " Exceeded device memory size limit!! Please cut to make it smaller! Current size is %.3f limit is %.3f" %(size_t, lim_53[owner]))
                    sys.exit(1)
                lim_i += 1
            else:
