...
static diag_cmd_reg_obj_t g_diag_default_cmd_tbl[] = {
    ...
    { DIAG_CMD_GET_INIT_INFO, DIAG_CMD_GET_INIT_INFO, diag_cmd_get_init_info},
#endif /* DIAG_SUPPORT_INIT_INFO */
...
static unsigned long g_dfx_osal_queue_id;
...
{
    uint8_t msg_data[DFX_MSG_MAX_SIZE + DFX_MSG_ID_LEN] = { 0 };
...
{
    errcode_t ret;
    ret = (errcode_t)osal_msg_queue_create("dfx_msg", DFX_QUEUE_MAX_SIZE, &g_dfx_osal_queue_id,
                                           0, DFX_MSG_MAX_SIZE + DFX_MSG_ID_LEN);
    if (ret != ERRCODE_SUCC) {
        return ERRCODE_FAIL;
    }

    osal_task *task = osal_kthread_create(msg_process_thread, NULL, "dfx_msg", 0x1000);
    if (task == NULL) {
        return ERRCODE_FAIL;
    }
    return ERRCODE_SUCC;
...
int msg_process_proc(uint32_t msg_id, uint8_t *data, uint16_t size)
{
    zdiag_dfx_rev_msg();
    switch (msg_id) {
