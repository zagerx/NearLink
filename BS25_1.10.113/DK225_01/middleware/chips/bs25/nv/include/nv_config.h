...
#define FLASH_PHYSICAL_ADDR_START     EMBED_FLASH_START
#else
#define FLASH_PHYSICAL_ADDR_START     DATA_FLASH_START
#endif
#define FLASH_PHYSICAL_ADDR_END       (FLASH_PHYSICAL_ADDR_START + EMBED_FLASH_LEN)

#ifdef USE_EMBED_FLASH
#define FLASH_MAPPED_ADDR_START       EMBED_FLASH_START
#define FLASH_MAPPED_END              (EMBED_FLASH_START + EMBED_FLASH_LEN)
/* NV运行区默认起始地址(物理地址) */
#define KV_STORE_START_ADDR           (EMBED_FLASH_START + 0x1FC000)
#else
#define FLASH_MAPPED_ADDR_START       DATA_FLASH_START
#define FLASH_MAPPED_END              (DATA_FLASH_START + EMBED_FLASH_LEN)
/* NV运行区默认起始地址(物理地址) */
#define KV_STORE_START_ADDR           (DATA_FLASH_START + 0xFC000)
#endif
...
/* NV运行区默认长度（不包含备份区） */
#define KV_STORE_DATA_SIZE            (KV_STORE_DATA_PAGE_NUM * FLASH_PAGE_SIZE)


/* NV备份区默认起始地址(物理地址) */
#define KV_BACKUP_START_ADDR          (KV_STORE_START_ADDR + KV_STORE_DATA_SIZE)
/* NV备份区page数 */
#define KV_BACKUP_PAGE_NUM            3
/* NV备份区默认长度 */
