...
static void pm_state_work_to_standby(void)
{
    ...
    uapi_pm_remove_sleep_veto(PM_VETO_SYS);
...
#else
    uapi_pm_add_sleep_veto(PM_VETO_SYS); // 睡眠否决：防止在处理期间CPU让出期间进睡眠
...
#else
    uapi_pm_add_sleep_veto(PM_VETO_SYS);
...
static void pm_sys_task(void const *arg)
{
    ...
    while (1) {
        ...
        if ((ret != OSAL_SUCCESS) || (mail.pm_state > PM_STATE_SLEEP_TO_WORK)) {
            continue;
        }

        uapi_pm_add_sleep_veto(PM_VETO_SYS);
        g_pm_state_trans_func[mail.pm_state](0);
        if ((mail.pm_state == PM_STATE_WORK_TO_STANDBY) || (mail.pm_state == PM_STATE_STANDBY_TO_SLEEP)) {
            uapi_pm_remove_sleep_veto(PM_VETO_SYS);
        }
