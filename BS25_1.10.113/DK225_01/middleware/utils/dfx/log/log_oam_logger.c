...
void log_event_print0(uint32_t log_header, uint32_t presspara)
{
    ...
    uint8_t level = getlog_level(presspara);
    level = sdt_to_diag_log_level(level);
    uint32_t msg_id = (presspara & ~OAM_LOG_LEVEL_MASK) | level;

    uapi_diag_report_sys_msg(module_id, msg_id, NULL, 0, level);
#endif
}
...
void log_event_print1(uint32_t log_header, uint32_t presspara, uint32_t para1)
{
    ...
    uapi_diag_report_sys_msg(module_id, msg_id, (uint8_t *)log_data, sizeof(log_data), level);
#endif
}
...
void log_event_print_alterable_para_press(uint32_t log_header, uint32_t presspara, uint32_t paranum, ...)
{
    ...
    uapi_diag_report_sys_msg(module_id, msg_id, (uint8_t *)log_data, sizeof(uint32_t) * uc_para_num, level);
#endif
}
