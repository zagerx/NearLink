...
errcode_t transmit_cmd_delete_file(uint8_t cmd_id, void *cmd_param, uint16_t cmd_param_size,
                                   diag_option_t *option, bool from_upper_machine)
{
    ...
    if (req->file_type == 0) {
        ret_value = dfx_file_delete((const char *)req->name);
    } else {
        ret_value = dfx_file_rmdir((const char *)req->name);
    }
