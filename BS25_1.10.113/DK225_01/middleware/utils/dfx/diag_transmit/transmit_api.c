...
STATIC errcode_t soc_dfx_save_file_emmc_async_local(const char *file_name, uint8_t *buf, uint32_t size,
...
STATIC errcode_t soc_dfx_save_file_async_remote(const char *file_name, uint8_t *buf, uint32_t size,
    save_file_result_hook handler, uintptr_t usr_data)
{
    dfx_assert(file_name);
    dfx_assert(buf);
...
STATIC errcode_t soc_dfx_save_file_emmc_async(const char *file_name, uint8_t *buf, uint32_t size,
    save_file_result_hook handler, uintptr_t usr_data)
{
#if (CONFIG_DFX_SUPPORT_FILE_SYSTEM == DFX_YES)
