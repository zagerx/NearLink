...
STATIC void report_ls_info(dfx_file_ls_node_info_t *info, uintptr_t usr_data)
{
                          ...
                          bool from_upper_machine)
{
    ...
    usr_param.idx = 0;
    if (dfx_file_ls((const char *)req->path, report_ls_info, (uintptr_t)&usr_param) != ERRCODE_SUCC) {
        dfx_log_err("[ERR]get file list failed!\r\n");
        return ERRCODE_FAIL;
    }

    /* Send a packet with all 0xFF to indicate the end. */
    dfx_file_ls_node_info_t ls_node = {0};
