...
/**
 * @if Eng
 * @brief  Length of device smp irk.
 * @else
 * @brief  蓝牙设备配对的IRK长度。
 * @endif
 */
#define BLE_PAIRED_IRK_LEN 16

/**
 * @if Eng
 * @brief BLE authentication result.
 * @else
 * @brief BLE鉴权结果。
 * @endif
 */
typedef struct {
    ...
    uint8_t ltk[BLE_PAIRED_LTK_LEN];            /*!< @if Eng link key.
                                                     @else   链路密钥。 @endif */
    uint8_t remote_id;                          /*!< @if Eng remote id, used to get pair info.
                                                     @else   对端设备编号，用于获取配对信息。 @endif */
} ble_auth_info_evt_t;

/**
 * @if Eng
 * @brief BLE pair infomation.
 * @else
 * @brief BLE配对信息。
 * @endif
 */
typedef struct {
    bd_addr_t local_addr;               /*!< @if Eng BLE local address.
                                             @else   BLE本地地址。 @endif */
    bd_addr_t remote_addr;              /*!< @if Eng BLE remote address.
                                             @else   BLE对端地址。 @endif */
    uint8_t ltk[BLE_PAIRED_LTK_LEN];    /*!< @if Eng BLE LTK.
                                             @else   BLE LTK。 @endif */
    uint8_t irk[BLE_PAIRED_IRK_LEN];    /*!< @if Eng BLE IRK.
                                             @else   BLE IRK。 @endif */
} ble_pair_info_t;
...
/**
 * @if Eng
 * @brief Use this funtion to get the pairing information of a specific device.
 * @par   Use this funtion to get the pairing information of a specific device.
 * @param  [in]  remote_id remote device index, see @ref ble_auth_info_evt_t.
 * @param  [out] pair_info pair information.
 * @retval pair state
 * @par Dependency:
 * @li  bts_def.h
 * @else
 * @brief  获取设备的配对信息。
 * @par    获取设备的配对信息。
 * @param  [in]  remote_id 对端设备编号，参考 @ref ble_auth_info_evt_t。
 * @param  [out] pair_info 配对信息.
 * @retval 配对状态。
 * @par 依赖:
 * @li  bts_def.h
 * @endif
 */
errcode_t gap_ble_get_pair_info_by_remote_id(uint8_t remote_id, ble_pair_info_t *pair_info);
