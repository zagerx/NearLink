...
/**
 * @if Eng
 * @brief  Seek result type.
 * @else
 * @brief  设备发现结果类型。
 * @endif
 */
typedef enum {
    SLE_SEEK_RESULT_TYPE_CONNECTABLE_ANNOUNCE_PACKET   = 0x1,   /*!< @if Eng connectable announce packet
                                                                     @else   可连接的广播包 @endif */
    SLE_SEEK_RESULT_TYPE_SCANABLE_ANNOUNCE_PACKET      = 0x2,   /*!< @if Eng scanable announce packet
                                                                     @else   可扫描的广播包 @endif */
    SLE_SEEK_RESULT_TYPE_DIRECT_ANNOUNCE_PACKET        = 0x4,   /*!< @if Eng direct announce packet
                                                                     @else   定向的广播包 @endif */
    SLE_SEEK_RESULT_TYPE_SEEK_RESPONCE_PACKET          = 0x8,   /*!< @if Eng seek response packet
                                                                     @else   扫描响应包 @endif */
    SLE_SEEK_RESULT_TYPE_FIRST_CHANNEL_ANNOUNCE_PACKET = 0x10,  /*!< @if Eng first channel announce packet
                                                                     @else   第一通道的广播包 @endif */
} sle_seek_result_type_t;

/**
 * @if Eng
 * @brief  Seek result data status.
 * @else
 * @brief  设备发现扫描结果数据状态。
 * @endif
 */
typedef enum {
    SLE_SEEK_RESULT_DATA_STATUS_COMPLETE,               /*!< @if Eng complete seek result data
                                                             @else   完整的扫描结果数据包 @endif */
    SLE_SEEK_RESULT_DATA_STATUS_INCOMPLETE_FOLLOWING,   /*!< @if Eng incomplete seek result data, this packet is
                                                                     following from previous packet
                                                             @else   不完整的扫描结果数据包，本包为前一包的后续报文 @endif */
    SLE_SEEK_RESULT_DATA_STATUS_INCOMPLETE_TRUNCATED,   /*!< @if Eng incomplete seek result data, this packet is truncated
                                                             @else   不完整的扫描结果数据包，本包被截断 @endif */
    SLE_SEEK_RESULT_DATA_STATUS_RFU,                    /*!< @if Eng RFU
                                                             @else   预留未来使用 @endif */
} sle_seek_result_data_status_t;
...
typedef struct sle_seek_result_info {
    uint8_t event_type;                /*!< @if Eng event type { @ref sle_seek_result_type_t }
                                            @else   上报事件类型 { @ref sle_seek_result_type_t } @endif */
    ...
    int8_t rssi;                       /*!< @if Eng rssi
                                            ...
                                            @endif */
    uint8_t data_status;               /*!< @if Eng data status { @ref sle_seek_result_data_status_t }
                                            @else   数据状态 { @ref sle_seek_result_data_status_t } @endif */
