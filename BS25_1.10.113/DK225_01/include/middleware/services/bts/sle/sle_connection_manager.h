...
/**
 * @if Eng
 * @brief SLE pair infomation.
 * @else
 * @brief SLE配对信息。
 * @endif
 */
typedef struct {
    sle_addr_t local_addr;              /*!< @if Eng SLE local address.
                                             @else   SLE本地地址。 @endif */
    sle_addr_t remote_addr;             /*!< @if Eng SLE remote address.
                                             @else   SLE对端地址。 @endif */
    uint8_t linkkey[SLE_LINK_KEY_LEN];  /*!< @if Eng SLE LTK.
                                             @else   SLE LTK。 @endif */
} sle_pair_info_t;
 ...
 * @param  [in]  remote_id remote device index, see @ref sle_auth_info_evt_t.
 ...
 * @brief  获取设备的配对信息。
 * @par    获取设备的配对信息。
 * @param  [in]  remote_id 对端设备编号，参考 @ref sle_auth_info_evt_t。
 ...
 * @endif
 */
errcode_t sle_get_pair_info_by_remote_id(uint8_t remote_id, sle_pair_info_t *pair_info);
